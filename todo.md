...
## 大功能迭代：Hermes Agent 功能集

### 工具系统（Function Calling）
- [ ] 实现 Function Calling 核心引擎（解析 tool_calls，执行工具，返回结果）
- [ ] 内置工具：web_search（DuckDuckGo）
- [ ] 内置工具：calculator（数学计算）
- [ ] 内置工具：datetime（当前时间/日期）
- [ ] 内置工具：weather（天气查询）
- [ ] 内置工具：url_fetch（抓取网页内容）
- [ ] 工具集管理页面（启用/禁用各工具集）
- [ ] 对话中显示工具调用过程和结果

### 持久记忆系统
- [ ] MEMORY.md 风格持久记忆存储（AsyncStorage）
- [ ] USER.md 风格用户画像存储
- [ ] 记忆管理页面（查看、编辑、删除记忆条目）
- [ ] 自动将记忆注入 System Prompt
- [ ] 对话结束后自动提取并保存新记忆

### 技能系统
- [ ] 技能数据结构设计（SKILL.md 格式）
- [ ] 内置技能库（翻译、总结、写代码、写作助手等）
- [ ] 技能中心页面（浏览、搜索技能）
- [ ] 技能创建/编辑页面
- [ ] 对话中通过 /skill 命令调用技能
- [ ] 技能启用/禁用管理
...
### 上下文文件
- [ ] 上下文文件管理（添加、编辑、删除项目背景文件）
- [ ] 自动注入上下文文件内容到 System Prompt

### MCP 集成
- [ ] MCP 服务器配置页面（添加/删除 MCP 服务器）
- [ ] MCP 工具发现和调用
- [ ] MCP 工具在工具集中显示

### 定时任务
- [ ] 定时任务创建页面（Cron 表达式/间隔设置）
- [ ] 定时任务列表管理（启用/暂停/删除）
- [ ] 后台任务执行（Expo Background Task）
